...
import { louvain } from "communities"

export function drawAll(divElIdMenu, divElIdSvg, lesmiserables, celegans, louvain_unweighted, louvain_weighted) {
    // global variables
    ...
    // menu
    const menuCanvas = d3.select(divElIdMenu)
    const svgCanvas = d3.select(divElIdSvg)
    // gui
    ...
    const dampConst = 100
    let damping = dampConst // damping constant
    const C = 0.4 //0.45 // 0.52 // 0.4537 // 3 // 0.399
    const Kf = C * Math.sqrt((width * height) / nodes.length) // Fruchterman-Reindold
    const Ka = 1.5 // 3.8 // ForceAtlas2
    const KgF = 30 //30 // 0.5  // gravitational force Fruchterman-Reindold
