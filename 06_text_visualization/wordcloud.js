...
let svgCanvas = undefined
let svg = undefined
let preGroup = undefined
let obbW = undefined
let aabbW = undefined
let words = undefined
...
export function drawAll(divElId, words) {
    canvas = d3.select(divElId)
    svgCanvas = canvas.append("div").attr("class", "cell").attr("id", "svg-canvas")
    svg = svgCanvas.append("svg").attr("width", width).attr("height", height)
    preGroup = svg.append("g").attr('class', 'pre-group')
