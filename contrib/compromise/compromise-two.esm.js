...
var utils = {
  ...
  terms: function(n3) {
    const m2 = this.match(".");
    return typeof n3 === "number" ? m2.eq(n3) : m2;
  },
  /** */
...
var View = class _View {
  ...
  update(pointer) {
    const m2 = new _View(this.document, pointer);
...
function mergeDeep(model5, plugin3) {
  if (isObject(plugin3)) {
    for (const key in plugin3) {
      if (isObject(plugin3[key])) {
        if (!model5[key]) Object.assign(model5, { [key]: {} });
        mergeDeep(model5[key], plugin3[key]);
      } else {
        Object.assign(model5, { [key]: plugin3[key] });
      }
    }
  }
  return model5;
}
function mergeQuick(model5, plugin3) {
  for (const key in plugin3) {
...
var extend = function(plugin3, world2, View2, nlp2) {
  if (isArray2(plugin3)) {
    plugin3.forEach((p5) => extend(p5, world2, View2, nlp2));
    return;
  }
  const { methods: methods17, model: model5, compute: compute4, hooks: hooks2 } = world2;
  if (plugin3.methods) {
    mergeQuick(methods17, plugin3.methods);
  }
  if (plugin3.model) {
    mergeDeep(model5, plugin3.model);
  }
  if (plugin3.irregulars) {
    addIrregulars(model5, plugin3.irregulars);
  }
  if (plugin3.compute) {
    Object.assign(compute4, plugin3.compute);
  }
  if (hooks2) {
    world2.hooks = hooks2.concat(plugin3.hooks || []);
  }
  if (plugin3.api) {
    plugin3.api(View2);
  }
  if (plugin3.lib) {
    Object.keys(plugin3.lib).forEach((k2) => nlp2[k2] = plugin3.lib[k2]);
  }
  if (plugin3.tags) {
    nlp2.addTags(plugin3.tags);
  }
  if (plugin3.words) {
    nlp2.addWords(plugin3.words);
  }
  if (plugin3.frozen) {
    nlp2.addWords(plugin3.frozen, true);
  }
  if (plugin3.mutate) {
    plugin3.mutate(world2, nlp2);
  }
};
...
nlp.plugin = function(plugin3) {
  extend_default(plugin3, this._world, View_default, this);
  return this;
};
...
fns3.replace = function(match2, input, keep) {
  ...
  this.soften();
  return m2.replaceWith(input, keep);
};
...
var contractions = (view) => {
  ...
  const list2 = model5.one.contractions || [];
...
var ifFn = function(regs, group, opts2) {
  ...
  ptrs = ptrs.filter((ptr, i3) => {
    ...
    const res = one.match(m2.docs, todo, cache2[i3]).ptrs;
    return res.length > 0;
  });
...
var ifNo = function(regs, group, opts2) {
  ...
  regs = parseRegs(regs, opts2, this.world);
  const cache2 = this._cache || [];
  const view = this.filter((m2, i3) => {
    const todo = { regs, group, justOne: true };
    const ptrs = one.match(m2.docs, todo, cache2[i3]).ptrs;
    return ptrs.length === 0;
  });
  if (this._cache) {
...
var methods7 = {};
methods7.splitOn = function(m2, group) {
  const { splitAll: splitAll2 } = this.methods.one.pointer;
  const splits = getDoc(m2, this, group).fullPointer;
...
methods7.splitBefore = function(m2, group) {
  const { splitAll: splitAll2 } = this.methods.one.pointer;
  const splits = getDoc(m2, this, group).fullPointer;
  const all4 = splitAll2(this.fullPointer, splits);
...
methods7.splitAfter = function(m2, group) {
  const { splitAll: splitAll2 } = this.methods.one.pointer;
  const splits = getDoc(m2, this, group).fullPointer;
  const all4 = splitAll2(this.fullPointer, splits);
  let res = [];
  ...
  doc.docs.forEach((terms, n3) => {
    for (let i3 = 0; i3 < terms.length; i3 += 1) {
      ...
      if (starts.hasOwnProperty(t3.id)) {
        ...
        const m2 = doc.update([[n3, i3, end2]]);
        text += terms[i3].pre || "";
        text += fn(m2);
        i3 = end2 - 1;
        text += terms[i3].post || "";
...
var doesMatch2 = function(full, m2) {
  return full[1] <= m2[1] && m2[2] <= full[2];
};
var splitAll = function(full, m2) {
  const byN = indexN(m2);
  const res = [];
  full.forEach((ptr) => {
    const [n3] = ptr;
...
var getUnion = function(a2, b) {
  ...
  both.forEach((ptr) => {
    ...
    const hmm = byN[n3].filter((m2) => doesOverlap(ptr, m2));
    hmm.push(ptr);
...
var getDoc3 = (m2, view) => {
  if (typeof m2 === "string" || isArray8(m2)) {
...
var methods15 = {};
methods15.union = function(m2) {
  m2 = getDoc3(m2, this);
  let ptrs = union_default(this.fullPointer, m2.fullPointer);
  ptrs = addIds3(ptrs, this.document);
  return this.toView(ptrs);
};
methods15.and = methods15.union;
methods15.intersection = function(m2) {
  m2 = getDoc3(m2, this);
  let ptrs = intersection_default(this.fullPointer, m2.fullPointer);
  ptrs = addIds3(ptrs, this.document);
  return this.toView(ptrs);
};
methods15.not = function(m2) {
  m2 = getDoc3(m2, this);
  let ptrs = difference_default(this.fullPointer, m2.fullPointer);
  ptrs = addIds3(ptrs, this.document);
  return this.toView(ptrs);
};
...
var lib_default3 = {
  ...
  buildNet: function(matches3) {
    ...
    const net2 = methods17.one.buildNet(matches3, this.world());
    net2.isNet = true;
    return net2;
  }
};
...
var api = function(View2) {
  View2.prototype.sweep = function(net2, opts2 = {}) {
    ...
    const { methods: methods17 } = world2;
    let found = methods17.one.bulkMatch(docs, net2, this.methods, opts2);
...
var runMatch2 = function(maybeList, document, docCache, methods17, opts2) {
  ...
  for (let n3 = 0; n3 < maybeList.length; n3 += 1) {
    for (let i3 = 0; i3 < maybeList[n3].length; i3 += 1) {
      const m2 = maybeList[n3][i3];
      const res = methods17.one.match([document[n3]], m2);
...
var sweep = function(document, net2, methods17, opts2 = {}) {
  const docCache = methods17.one.cacheDoc(document);
  let maybeList = getHooks_default(docCache, net2.hooks);
...
var hasHyphen2 = function(str, model5) {
  ...
  const { prefixes, suffixes: suffixes4 } = model5.one;
  ...
  if (suffixes4.hasOwnProperty(parts[1])) {
    return false;
  }
...
var prepare = function(words = [], opts2 = {}) {
  ...
  Object.keys(prefixes).forEach((str) => {
    ...
    model5.one.typeahead[str] = prefixes[str];
  });
  return this;
};
var lib_default5 = {
...
var symbols = function(t3) {
  ...
  for (let i3 = 0; i3 < t3.nodes.length; i3++) {
    const m2 = reSymbol.exec(t3.nodes[i3]);
    if (!m2) {
      t3.symCount = i3;
      break;
    }
    t3.syms[encoding_default.fromAlphaCode(m2[1])] = encoding_default.fromAlphaCode(m2[2]);
  }
...
Object.keys(data_default).forEach((tag) => {
  ...
  Object.keys(wordsObj).forEach((w) => {
    switches[w] = tag;
    if (tag === "Noun|Verb") {
      const plural = toPlural_default(w, tmpModel);
...
var conjugate = function(inf, model5) {
  const { toPast: toPast2, toPresent: toPresent2, toGerund: toGerund2, toParticiple: toParticiple2 } = model5.two.models;
  if (inf === "be") {
    return {
      Infinitive: inf,
      Gerund: "being",
  ...
  const found = {
    Infinitive: str,
    PastTense: convert_default(str, toPast2),
    PresentTense: convert_default(str, toPresent2),
    Gerund: convert_default(str, toGerund2),
...
var suffixLoop = function(str = "", suffixes4 = []) {
  const len = str.length;
  const max3 = len <= 6 ? len - 1 : 6;
  for (let i3 = max3; i3 >= 1; i3 -= 1) {
    const suffix = str.substring(len - i3, str.length);
    if (suffixes4[suffix.length].hasOwnProperty(suffix) === true) {
      const pre = str.slice(0, len - i3);
      const post = suffixes4[suffix.length][suffix];
      return pre + post;
    }
  }
  return null;
...
var byTag_default = {
  ...
  Singular: (word, lex, methods17, model5) => {
    ...
    const plural = methods17.two.transform.noun.toPlural(word, model5);
    if (!already[plural]) {
...
var suffixLoop2 = function(str = "", suffixes4 = []) {
  const len = str.length;
  let max3 = 7;
  if (len <= max3) {
    max3 = len - 1;
  ...
  for (let i3 = max3; i3 > 1; i3 -= 1) {
    ...
    if (suffixes4[suffix.length].hasOwnProperty(suffix) === true) {
      const tag = suffixes4[suffix.length][suffix];
      return tag;
    }
  }
  return null;
...
var prefixLoop = function(str = "", prefixes = []) {
  ...
  for (let i3 = max3; i3 > 2; i3 -= 1) {
    ...
    if (prefixes[prefix5.length].hasOwnProperty(prefix5) === true) {
      const tag = prefixes[prefix5.length][prefix5];
      return tag;
    }
  }
  return null;
...
var postPunct = /[,)"';:\-–—.…]/;
var setContraction = function(m2, suffix) {
  if (!m2.found) {
    return;
  }
...
var getWords = function(net2) {
  return Object.keys(net2.hooks).filter((w) => !w.startsWith("#") && !w.startsWith("%"));
};
var maybeMatch = function(doc, net2) {
  const words = getWords(net2);
  if (words.length === 0) {
    return doc;
  }
  if (!doc._cache) {
...
var lazyParse = function(input, reg) {
  ...
  const m2 = maybeMatch_default(doc, net2);
  if (m2.found) {
...
var matchVerb = function(m2, lemma) {
  const conjugate2 = m2.methods.two.transform.verb.conjugate;
  const all4 = conjugate2(lemma, m2.model);
  if (m2.has("#Gerund")) {
    return all4.Gerund;
  }
  if (m2.has("#PastTense")) {
...
var swapNoun = function(m2, lemma) {
  let str = lemma;
  if (m2.has("#Plural")) {
...
var swapAdverb = function(m2, lemma) {
...
var swapAdjective = function(m2, lemma) {
  const { toComparative: toComparative3, toSuperlative: toSuperlative3 } = m2.methods.two.transform.adjective;
  let str = lemma;
  if (m2.has("#Comparative")) {
    str = toComparative3(str, m2.model);
  } else if (m2.has("#Superlative")) {
    str = toSuperlative3(str, m2.model);
  }
  if (str) {
    m2.replaceWith(str);
  }
...
var swap = function(from, to, tag) {
  ...
  if (m2.has("#Verb")) {
    return swap_verb_default(m2, to);
  }
  if (m2.has("#Noun")) {
